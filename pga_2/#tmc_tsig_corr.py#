...
def main():
    ...
    ##################################################################################
    # Let's plot the zero point drift
    zero_time,zero_meas = tmc_parse_data.tmc_parse_data(tmc_file,'ZERO','ADC'+adc)
    zero_mvavg = moving_average(zero_meas,40)
    mean_sub_zero = mean_sub(zero_mvavg)
    
    print 'Plotting mean subtracted zero'
    plt.plot(zero_time,zero_meas,color='blue')
    plt.plot(zero_time,zero_mvavg,color='red')
    plt.show()
